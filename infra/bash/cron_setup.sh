...
echo "*/5 * * * * cd \$PROJECT_DIR && ./infra/bash/verify_multi_chain.sh --check-only --quiet >> \$PROJECT_DIR/data/logs/verify.log 2>&1" >> "$TEMP_CRON_FILE"

echo "# Rotate logs daily" >> "$TEMP_CRON_FILE"
echo "0 0 * * * find \$PROJECT_DIR/data/logs -name \"*.log\" -mtime +7 -exec rm {} \; >> \$PROJECT_DIR/data/logs/log_rotation.log 2>&1" >> "$TEMP_CRON_FILE"

echo "# Backup configuration weekly" >> "$TEMP_CRON_FILE"
echo "0 1 * * 0 mkdir -p \$PROJECT_DIR/data/backups && tar -czf \$PROJECT_DIR/data/backups/config_backup_\$(date +\%Y\%m\%d).tar.gz \$PROJECT_DIR/config/ >> \$PROJECT_DIR/data/logs/backup.log 2>&1" >> "$TEMP_CRON_FILE"

echo "# Rotate backups monthly" >> "$TEMP_CRON_FILE"
echo "0 2 1 * * find \$PROJECT_DIR/data/backups -name \"config_backup_*.tar.gz\" -mtime +30 -exec rm {} \; >> \$PROJECT_DIR/data/logs/backup_rotation.log 2>&1" >> "$TEMP_CRON_FILE"

echo "# Update system packages weekly" >> "$TEMP_CRON_FILE"
echo "0 3 * * 0 apt-get update && apt-get upgrade -y >> \$PROJECT_DIR/data/logs/system_update.log 2>&1" >> "$TEMP_CRON_FILE"
...
if [ -n "$CHAINS" ]; then
    ...
    for chain_id in "${CHAIN_ARRAY[@]}"; do
        ...
        echo "*/10 * * * * curl -s http://localhost:5001/api/check_chain/$chain_id >> \$PROJECT_DIR/data/logs/chain_${chain_id}_check.log 2>&1" >> "$TEMP_CRON_FILE"
    done
fi
