...
class Configuration:
    ...
    def __init__(self, config_path=None, env_file=None, skip_env=False):
        ...
        """
        logger.debug(
            "Configuration.__init__ called with config_path=%s, env_file=%s, skip_env=%s",
            config_path,
            env_file,
            skip_env,
        )
        ...
        logger.debug(
            "Configuration initialized. DEBUG=%s, MIN_PROFIT=%s",
            self._config.get("DEBUG"),
            self._config.get("MIN_PROFIT"),
        )
    ...
    def __setattr__(self, name, value):
        ...
        if name.startswith("_") or name == "config_path":
            ...
            # Store all other attributes, including BASE_PATH, in the config
            # dict
    ...
    def _load_from_env(self):
        ...
        for key in self._config:
            ...
            if env_value is not None and env_value.strip() != "":  # Skip empty strings
                # Convert environment string to appropriate type based on
                # default
                ...
                if isinstance(default_value, bool):
                    ...
                    try:
                        ...
                        logger.warning(
                            f"Failed to convert env var '{key}' value '{env_value}' to int, using default {default_value}"
                        )
                    ...
                    try:
                        ...
                        logger.warning(
                            f"Failed to convert env var '{key}' value '{env_value}' to float, using default {default_value}"
                        )
                else:
        ...
        for key, value in os.environ.items():
            if (
                key not in self._config and key.isupper() and value.strip() != ""
            ):  # Only consider uppercase environment variables with non-empty values
...
class APIConfig:
    ...
    async def initialize(self) -> None:
        ...
        # Set these paths in the configuration if they're defined in
        # environment variables
        if token_addresses_path:
            self.cfg.TOKEN_ADDRESSES = token_addresses_path
        elif not hasattr(self.cfg, "TOKEN_ADDRESSES"):
            # Default fallback
            self.cfg.TOKEN_ADDRESSES = os.path.join(
                self.cfg.BASE_PATH, "resources/tokens/chainid-1/address2token.json"
            )
        ...
        if token_symbols_path:
            ...
            # Default fallback
            self.cfg.TOKEN_SYMBOLS = os.path.join(
                self.cfg.BASE_PATH, "resources/tokens/chainid-1/symbol2address.json"
            )
        ...
        if address_to_symbol_path:
            ...
            # Default fallback
            self.cfg.ADDRESS_TO_SYMBOL = os.path.join(
                self.cfg.BASE_PATH, "resources/tokens/chainid-1/address2symbol.json"
            )
    ...
    async def _populate_token_maps(self) -> None:
        ...
        if hasattr(self.cfg, "ADDRESS_TO_SYMBOL"):
            addr_to_sym = (
                await self.cfg._load_json_safe(self.cfg.ADDRESS_TO_SYMBOL) or {}
            )
            ...
            async with provider.limiter:
                ...
                try:
                    async with self._session.get(
                        url, params=params, headers=self._headers(provider)
                    ) as r:
                        if r.status == 429:
                        ...
                        if r.status >= 400:
                            logger.debug(
                                "%s HTTP %s – %s", provider.name, r.status, url
                            )
                            return None
                        return await r.json()
    ...
    async def get_real_time_price(
        self, token_or_addr: str, vs: str = "usd"
    ) -> Optional[Decimal]:
        ...
        if not prices:
            ...
            if fallback_price is not None:
                logger.info(
                    f"Using fallback price for {t_norm}: {fallback_price} {
                        vs.upper()} (all API providers failed)"
                )
            ...
            logger.warning(
                f"Failed to get price for {t_norm} - all providers failed and no fallback available"
            )
            return None
    ...
    async def _price_from_provider(
        ...
        try:
            # Special handling for ETH and BTC since they often need custom
            # handling
            if token.upper() in ["ETH", "BTC", "WETH", "WBTC"]:
                # Use provider-specific approach for these major tokens
                if prov.name == "binance":
                    # For ETH and BTC on Binance, use USDT pairs which are more
                    # reliable
                    # Remove W prefix for wrapped tokens
                    symbol = f"{token.upper().replace('W', '')}USDT"
                    data = await self._request(
                        prov, prov.price_url, params={"symbol": symbol}
                    )
                ...
                # For other providers, continue with standard approach but with
            ...
            if prov.name == "coinpaprika":
                try:
                    # For native tokens like ETH, BTC, use different API
                    # endpoints
                    ...
                    if (
                        data
                        and "quotes" in data
                        and vs.upper() in data["quotes"]
                        and "price" in data["quotes"][vs.upper()]
                    ):
    ...
    async def _volume_from_provider(
        self, prov: Provider, token: str
    ) -> Optional[float]:
        if prov.name == "binance":
            endpoint = prov.volume_url
            data = await self._request(
                prov, endpoint, params={"symbol": token + "USDT"}
            )
        ...
        if prov.name == "coingecko":
            ...
            data = await self._request(
                prov, endpoint.format(id=token_id), params=params
            )
    ...
    async def _hist_from_provider(
        self, prov: Provider, token: str, days: int
    ) -> List[float]:
        if prov.name == "coingecko":
            token_id = self.symbol_to_api_id.get(token, token.lower())
            data = await self._request(
    ...
    async def get_price_history(
        self, token_or_addr: str, timeframe: str = "1h", vs: str = "usd"
    ) -> List[Dict[str, Any]]:
        ...
        for prov in self.providers.values():
            ...
            if data:
                ...
                if formatted_data:
                    ...
                    logger.debug(
                        f"Loaded historical price data for {t_norm} from {
                            prov.name}: {
                            len(formatted_data)} points"
                    )
                ...
                try:
                    ...
                    if data and "pair" in data and "priceUsd" in data["pair"]:
                        ...
                        if current_price > 0:
                            ...
                            logger.debug(
                                f"Generated synthetic price history for {t_norm} from Dexscreener data"
                            )
                            return price_data
                except Exception as e:
            ...
            if current_price:
                ...
                for i in range(points):
                    ...
                    random_change = (
                        (random.random() - 0.45) * volatility * base_price
                    )  # Slight upward bias
    ...
    async def get_price(
        ...
        """Alias for get_real_time_price for compatibility with existing
        tests."""
    ...
    def _get_fallback_price(self, token: str,
        ...
        if token_upper in fallbacks:
            logger.debug(
                f"Using fallback price for {token_upper}: {
                    fallbacks[token_upper]} {
                    vs.upper()}"
            )
