...
class MultiChainCore:
    ...
    def __init__(self, config):
        ...
        self.health_check_interval = getattr(config, "HEALTH_CHECK_INTERVAL", 60)  # seconds
    ...
    def _parse_chains_config(self) -> List[Dict[str, Any]]:
        ...
        for chain in chains:
            ...
            missing_fields = [field for field in required_fields if not chain.get(field)]
            
            if missing_fields:
                logger.error(f"Chain config missing required fields: {', '.join(missing_fields)}")
                continue
    ...
    async def initialize(self) -> bool:
        ...
        """
        try:
            logger.info(f"Initializing multi-chain core with {len(self.chains_config)} chains")
            ...
            for i, result in enumerate(results):
                ...
                if isinstance(result, Exception):
                    logger.error(f"Failed to initialize chain worker for {self.chains_config[i].get('CHAIN_NAME', chain_id)}: {result}")
                    # Detailed error tracking
                    import datetime, traceback as _tb
                    error_entry = {
                        'timestamp': datetime.datetime.utcnow().isoformat(),
                        'error': str(result),
                        'traceback': _tb.format_exception(type(result), result, result.__traceback__)
                    }
            ...
            if success_count > 0:
                logger.info(f"Successfully initialized {success_count}/{len(self.chains_config)} chains")
    ...
    async def _init_chain_worker(self, chain_id: str, chain_config: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if await worker.initialize():
                ...
                logger.info(f"Initialized chain worker for {chain_config.get('CHAIN_NAME', chain_id)}")
                return True
            else:
                logger.error(f"Failed to initialize chain worker for {chain_config.get('CHAIN_NAME', chain_id)}")
                return False
        except Exception as e:
            logger.exception(f"Exception initializing chain worker for {chain_config.get('CHAIN_NAME', chain_id)}: {e}")
            raise
    ...
    async def _check_chain_health(self, chain_id: str, worker: ChainWorker) -> Dict[str, Any]:
        ...
        while not self._shutdown_event.is_set():
            try:
                ...
                if health["status"] != "healthy":
                    logger.warning(f"Health check: System status is {health['status']}. "
                                  f"Active chains: {health['active_chains']}/{health['total_chains']}")
                    
                    # Log specific chain issues
                    for chain_id, chain_health in health["chains"].items():
                        if chain_health["status"] != "healthy":
                            logger.warning(f"Chain {chain_id} status: {chain_health['status']}")
                ...
                for chain_id, chain_health in health["chains"].items():
                    if chain_health["status"] == "unhealthy" and chain_id in self.workers:
