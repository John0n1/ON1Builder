...
class NonceCore:
    ...
    def __init__(
        ...
        self._cache_ttl = getattr(config, "NONCE_CACHE_TTL", 60)
        self._retry_delay = getattr(config, "NONCE_RETRY_DELAY", 1)
        self._max_retries = getattr(config, "NONCE_MAX_RETRIES", 5)
        self._tx_timeout = getattr(config, "NONCE_TRANSACTION_TIMEOUT", 120)
    ...
    async def track_transaction(
        ...
        """
        Keep an eye on a sent transaction so we can reset the nonce if it fails.
        """
        if address is None:
            if not hasattr(self.account, "address"):
        ...
        while True:
            try:
                ...
                if retries >= self._max_retries:
                    ...
                    return
                logger.warning("Error monitoring %s (%d/%d): %s",
                               tx_hash, retries, self._max_retries, e)
            await asyncio.sleep(self._retry_delay)

    async def wait_for_transaction(self, tx_hash: str, timeout: Optional[int] = None) -> bool:
