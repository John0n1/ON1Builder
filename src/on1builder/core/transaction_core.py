...
class TransactionCore:
    ...
    async def build_transaction(
        ...
        if nonce is None:
            ...
            if nonce is None:
                try:
                    nonce = await self.web3.eth.get_transaction_count(self.address, 'pending')
                except Exception as e:
                    logger.error(f"Web3 nonce failed: {e}; using latest")
                    try:
                        nonce = await self.web3.eth.get_transaction_count(self.address, 'latest')
                    except Exception:
        ...
        if gas_price is None:
            try:
                net_gas = self.web3.eth.gas_price
    ...
    async def simulate_transaction(self, tx: Dict[str, Any]) -> Tuple[bool, str, Optional[Dict[str, Any]]]:
        """
        ...
        try:
            ...
            if "to" in tx and tx["to"]:
                try:
                    ...
                    if code == b'':
                        return False, "Target address has no contract code", simulation_data
                except Exception:
                    pass
                    
            logger.debug(f"Transaction simulation successful: {result.hex() if result else 'no result'}")
            ...
            simulation_data = {"error_type": "simulation_failure", "raw_error": error_msg}
    ...
    async def optimize_gas_price(
        ...
        try:
            ...
            config_multiplier = self.configuration.get("GAS_PRICE_MULTIPLIER", 1.1)
            ...
            logger.debug(f"Gas price optimized: {current_gas_price} -> {optimized_gas_price} (x{total_multiplier:.2f})")
            return optimized_tx
    ...
    async def estimate_transaction_cost(self, tx: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            # Estimate gas
            gas_estimate = await self.web3.eth.estimate_gas(tx)
            gas_price = tx.get("gasPrice") or self.web3.eth.gas_price
            ...
            if self.api_config:
                try:
                    eth_price_data = await self.api_config.get_aggregated_price("ethereum")
            ...
            cost_estimate = {
                ...
                "transaction_value_eth": float(Decimal(tx.get("value", 0)) / Decimal(10**18)),
                "total_cost_wei": gas_cost_wei + tx.get("value", 0),
                "total_cost_eth": float((Decimal(gas_cost_wei) + Decimal(tx.get("value", 0))) / Decimal(10**18))
            }
            
            if eth_price_usd:
                cost_estimate["gas_cost_usd"] = float(gas_cost_eth) * eth_price_usd
                cost_estimate["total_cost_usd"] = cost_estimate["total_cost_eth"] * eth_price_usd
    ...
    async def prepare_flashloan_transaction(
        ...
        try:
            ...
            logger.info(f"Preparing Aave V3 flashloan: {len(assets)} assets, total value: {sum(amounts)}")
            
            # Ensure we have ABIRegistry
            if not self.abi_registry:
                raise StrategyExecutionError("ABIRegistry not available - cannot prepare flashloan")
            ...
            try:
                ...
                if contract_code == b'':
                    raise StrategyExecutionError(f"No contract code at address {flashloan_contract}")
            except Exception as e:
                raise StrategyExecutionError(f"Failed to validate flashloan contract: {e}")
            ...
            for i, asset in enumerate(assets):
                try:
                    ...
                    if asset_code == b'':
                        raise StrategyExecutionError(f"Asset {asset} is not a valid contract")
                        
                    # Check asset is available for flashloan
                    available = await self._check_aave_asset_availability(asset, amounts[i])
                    if not available:
                        raise StrategyExecutionError(f"Insufficient liquidity for asset {asset}")
                        
                    validated_assets.append(asset)
                    logger.debug(f"Validated asset {i+1}/{len(assets)}: {asset}")
                    
                except Exception as e:
                    raise StrategyExecutionError(f"Asset validation failed for {asset}: {e}")
    ...
    async def execute_flashloan(
        ...
        try:
            ...
            logger.info(f"Executing flashloan: {len(assets)} assets from contract {contract_address}")
            ...
            if self.db_manager and hasattr(self.db_manager, "store_flashloan"):
                try:
                    await self.db_manager.store_flashloan({
                        "tx_hash": tx_hash,
                        "assets": assets,
                        "amounts": amounts,
        ...
        **kwargs
        ...
        try:
            ...
            if strategy_function:
                try:
                    strategy_result = await strategy_function(flashloan_data, strategy_params)
    ...
    async def _ensure_flashloan_contract_deployed(self) -> str:
        ...
        try:
            ...
            configured_address = self.configuration.get("FLASHLOAN_CONTRACT_ADDRESS")
            if configured_address:
                # Validate existing contract
                try:
                    code = await self.web3.eth.get_code(configured_address)
                    if code != b'':
                        logger.debug(f"Using configured flashloan contract: {configured_address}")
                        return configured_address
                except Exception:
                    logger.warning(f"Configured flashloan contract {configured_address} not valid")
    ...
    async def _deploy_flashloan_contract(self) -> str:
        ...
        try:
            if not self.abi_registry:
                raise StrategyExecutionError("ABIRegistry required for contract deployment")
            ...
            addresses_provider = self.configuration.get(
            ...
            logger.warning("Contract deployment requires bytecode - using placeholder address")
    ...
    async def _check_aave_asset_availability(self, asset: str, amount: int) -> bool:
        ...
        try:
            ...
            pool_address = self.configuration.get(
                "AAVE_POOL_ADDRESS",
    ...
    async def _validate_flashloan_safety(self, flashloan_data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if eth_balance < min_eth_balance:
                raise StrategyExecutionError(f"Insufficient ETH for gas fees: {eth_balance}")
                
            # Validate flashloan amounts are reasonable
            for i, amount in enumerate(amounts):
                if amount <= 0:
                    raise StrategyExecutionError(f"Invalid amount for asset {i}: {amount}")
                    
                # Check amount isn't excessively large (basic sanity check)
                max_amount = self.configuration.get("MAX_FLASHLOAN_AMOUNT", 10**12)  # 1M tokens with 6 decimals
                if amount > max_amount:
                    raise StrategyExecutionError(f"Amount too large for asset {i}: {amount}")
            ...
            if self.safety_net:
                ...
                safe, details = await self.safety_net.check_transaction_safety(mock_tx)
                if not safe:
                    raise StrategyExecutionError(f"SafetyNet blocked flashloan: {details}")
    ...
    async def get_flashloan_contract_info(self, contract_address: str) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            try:
                ...
                if hasattr(contract.functions, 'ADDRESSES_PROVIDER'):
                    info["addresses_provider"] = await contract.functions.ADDRESSES_PROVIDER().call()
    ...
    async def monitor_flashloan_execution(
        ...
        try:
            ...
            if not receipt:
                raise asyncio.TimeoutError(f"Transaction {tx_hash} not confirmed within {timeout}s")
            ...
            if receipt["status"] == 1:
                ...
                execution_analysis["gas_cost_eth"] = float(Decimal(gas_cost) / Decimal(10**18))
                
            # Parse logs for flashloan events
            if receipt["logs"]:
                try:
                    flashloan_events = await self._parse_flashloan_events(receipt["logs"], flashloan_data)
            ...
            if self.db_manager and hasattr(self.db_manager, "update_flashloan"):
                try:
                    await self.db_manager.update_flashloan(tx_hash, execution_analysis)
                except Exception as e:
            ...
            logger.info(f"Flashloan monitoring complete: {execution_analysis['status']}")
    ...
    async def _parse_flashloan_events(
        ...
        try:
            ...
            contract = self.web3.contract(
                address=flashloan_data["flashloan_contract"],
                abi=flashloan_abi
            )
    ...
    async def recover_failed_flashloan(
        ...
        try:
            ...
            try:
                ...
                return {"error": "Cannot get transaction receipt", "recovery": "impossible"}
        ...
        try:
            ...
            for asset in assets:
                try:
                    ...
                    if balance > 0:
                        logger.info(f"Found stuck tokens {asset}: {balance}, attempting withdrawal")
                        ...
                        withdraw_tx = await self.build_transaction(
                            function_call=flashloan_contract.functions.withdrawToken(asset, balance)
                        )
    ...
    async def validate_contract_integration(self) -> Dict[str, bool]:
        ...
        try:
            ...
            logger.info(f"Contract integration validation: {passed}/{total} checks passed")
    ...
    async def test_flashloan_readiness(self, test_asset: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if not test_asset:
                test_asset = "0xA0b86a33E6441b4fa13647Bc13a7B7D8b8D5B3A5"  # USDC mainnet
            ...
            try:
                available = await self._check_aave_asset_availability(test_asset, test_amount)
            ...
            try:
                ...
                test_results["preparation"] = {"success": True, "data_keys": list(flashloan_data.keys())}
            except Exception as e:
            ...
            validation_passed = sum(test_results["validation"].values()) >= 6  # At least 6/8 checks
            asset_ready = test_results["asset_test"].get("available", False)
            prep_ready = test_results["preparation"]["success"]
            
            test_results["overall_ready"] = validation_passed and asset_ready and prep_ready
            
            logger.info(f"Flashloan readiness test completed: {test_results['overall_ready']}")
            return test_results
    ...
    async def get_integration_status(self) -> Dict[str, Any]:
        ...
        try:
            status = {
                ...
                "components": {
                    "nonce_core": self.nonce_core is not None,
                    "safety_net": self.safety_net is not None,
                    "abi_registry": self.abi_registry is not None,
                    "db_manager": self.db_manager is not None,
                    "api_config": self.api_config is not None,
                    "notification_manager": hasattr(self, 'notification_manager') and self.notification_manager is not None
                },
                "balances": {},
                "configuration": {
                    "gas_price_multiplier": self.configuration.get("GAS_PRICE_MULTIPLIER", 1.1),
                    "flashloan_contract": self.configuration.get("FLASHLOAN_CONTRACT_ADDRESS"),
                    "aave_pool": self.configuration.get("AAVE_POOL_ADDRESS"),
                    "profit_receiver": self.configuration.get("PROFIT_RECEIVER")
                }
            }
            ...
            if self.abi_registry:
                status["contract_validation"] = await self.validate_contract_integration()
            ...
            for tx_hash, tx_data in list(self._pending_txs.items()):
                try:
                    if "nonce" in tx_data.get("tx", {}):
                        ...
                        logger.info(f"Cancelled pending tx {tx_hash} with {cancel_hash}")
