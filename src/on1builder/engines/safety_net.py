...
class SafetyNet:
    ...
    async def stop(self) -> None:
        """Stop the safety net system. Alias for close()."""
        await self.close()
    ...
    async def is_safe_to_proceed(self) -> bool:
        ...
        min_balance_wei = self.web3.to_wei(self.config.get("MIN_BALANCE", 0.001), "ether")
        
        if balance < min_balance_wei:
            reason = f"Account balance ({self.web3.from_wei(balance, 'ether'):.4f} ETH) below minimum threshold"
            await self.break_circuit(reason)
            return False
        
        # Check gas price
        gas_price = await self.web3.eth.gas_price
        max_gas_price = self.config.get("MAX_GAS_PRICE", 500_000_000_000)  # Default 500 gwei
        
        if gas_price > max_gas_price:
            reason = f"Gas price ({self.web3.from_wei(gas_price, 'gwei'):.1f} gwei) above maximum threshold"
    ...
    async def break_circuit(self, reason: str) -> None:
        ...
        try:
            ...
            logger.warning("Notification system not available for circuit breaker alerts")
    ...
    async def is_transaction_duplicate(self, tx_hash: str) -> bool:
        ...
        async with self._cache_lock:
            ...
            if current_time > self._cache_expiry:
                ...
                self._cache_expiry = current_time + self.config.get("SAFETYNET_CACHE_TTL", 300)
    ...
    async def get_dynamic_gas_price(self) -> float:
        ...
        try:
                ...
                try:
                    ...
                    if oracle_abi:
                        oracle = self.web3.eth.contract(
                            address=self.web3.to_checksum_address(gas_price_oracle_address),
                            abi=oracle_abi
                        )
                        
                        # Call oracle method - depends on specific oracle contract
                        if hasattr(oracle.functions, "getLatestGasPrice"):
                            gas_price = await oracle.functions.getLatestGasPrice().call()
            ...
            if eip1559_supported:
                ...
                # Use base fee + priority fee with adjustments based on network congestion
                ...
                logger.debug(f"EIP-1559 gas price: base={base_fee_gwei:.2f} + priority={adjusted_priority:.2f} = {gas_gwei:.2f} gwei")
            else:
                # For non-EIP-1559 chains, adjust based on congestion
                congestion = await self.get_network_congestion()
                gas_gwei = gas_gwei * (1 + congestion * 0.5)
                logger.debug(f"Legacy gas price: {gas_gwei:.2f} gwei (congestion: {congestion:.2f})")
    ...
    async def adjust_slippage_tolerance(self, congestion_level: float = None) -> float:
        ...
        try:
            ...
            slippage_extreme = self.config.get("SLIPPAGE_EXTREME_CONGESTION", 2.0) # 2.0%
            ...
            # Further adjust based on token volatility if api_config is available
            ...
            if self.api_config and token_address:
                try:
                    ...
                    if hasattr(MarketMonitor, "check_market_conditions"):
                        ...
                        if market_monitor:
                            volatility_data = await market_monitor.check_market_conditions(token_address, "volatility")
                            volatility_condition = volatility_data.get("condition", "medium")
            ...
            logger.debug(f"Adjusted slippage tolerance: {slippage:.2f}% (congestion: {congestion_level:.2f})")
            return slippage
    ...
    async def _calculate_gas_cost(self, gas_price: float, gas_used: int) -> float:
        ...
        try:
            ...
            logger.debug(f"Calculated gas cost: {cost_eth:.6f} ETH (gas: {gas_used}, price: {gas_price:.2f} gwei)")
            return cost_eth
        except Exception as e:
    ...
    async def _calculate_profit(self, amountIn: float, amountOut: float, gas_cost: float) -> float:
        ...
        try:
            ...
            # Apply safety margin
            safety_margin = self.config.get("PROFIT_SAFETY_MARGIN", 0.9)  # 10% safety margin
            adjusted_profit = net_profit * safety_margin
            
            logger.debug(f"Calculated profit: {adjusted_profit:.6f} ETH (raw: {raw_profit:.6f}, gas: {gas_cost:.6f})")
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    if hasattr(self.web3, "geth") and hasattr(self.web3.geth, "txpool"):
                        ...
                        pending_count = int(txpool.pending, 16) if hasattr(txpool, "pending") else 0
                except Exception:
                    # Unable to get pending transactions
                    pass
            
            # Normalize pending count (0-1)
            pending_factor = min(1.0, pending_count / 5000)  # Assume 5000+ is very congested
            ...
            try:
                ...
                if len(self._gas_price_history) >= 2:
                    recent_avg = sum(self._gas_price_history[-3:]) / min(3, len(self._gas_price_history))
                    older_avg = sum(self._gas_price_history[:-3]) / max(1, len(self._gas_price_history) - 3)
                    
                    if older_avg > 0:
                        # Normalize trend between 0-1 (0 = decreasing, 1 = increasing sharply)
            ...
            # Combine metrics
            congestion = (gas_ratio * 0.5) + (pending_factor * 0.3) + (gas_price_trend * 0.2)
            
            # Ensure range is 0-1
            congestion = max(0.0, min(1.0, congestion))
            
            # Implement proper historical tracking with exponential moving average
            ...
            logger.debug(f"Network congestion: {congestion:.2f} (gas ratio: {gas_ratio:.2f}, pending: {pending_count}, trend: {gas_price_trend:.2f})")
            return congestion
    ...
    async def ensure_profit(self, transaction_data: Dict[str, Any]) -> bool:
        """
        Ensure a transaction is profitable considering gas costs and market conditions.
        ...
        """
        try:
            # API config should already be initialized during SafetyNet.initialize
            
            # Check if api_config initialization succeeded
            if not self.api_config:
                logger.error("API config not available for profit calculation - cannot proceed")
                return False
            ...
            if not input_token or not output_token:
                gas_price = transaction_data.get('gas_price', await self.get_dynamic_gas_price())
                ...
                logger.debug(f"Profit check (ETH): {profit:.6f} ETH {'≥' if is_profitable else '<'} {min_profit:.6f} ETH minimum")
            ...
            output_price = await self.api_config.get_real_time_price(output_token, "eth")
            
            if not input_price or not output_price:
                logger.warning(f"Could not get token prices for {input_token} or {output_token}")
                return False
            ...
            # Get gas costs
            gas_price = transaction_data.get('gas_price', await self.get_dynamic_gas_price())
            gas_used = transaction_data.get('gas_used', 150000)  # Higher default for token transactions
            gas_cost = await self._calculate_gas_cost(gas_price, gas_used)
            ...
            logger.debug(f"Profit check (tokens): {adjusted_profit:.6f} ETH {'≥' if is_profitable else '<'} {min_profit:.6f} ETH minimum")
            
            # Include more details in logs if profitable
            if is_profitable:
                logger.info(f"Profitable transaction: {input_token}->{output_token}, in: {amount_in}, out: {amount_out}, profit: {adjusted_profit:.6f} ETH")
    ...
    async def check_transaction_safety(self, tx_data: Dict[str, Any]) -> Tuple[bool, Dict[str, Any]]:
        """
        Check if a transaction is safe to execute, considering multiple factors.
        ...
        try:
            ...
            result["check_details"]["profit_check"] = {
                "passed": profit_ok
            }
            ...
            try:
                ...
                result["check_details"]["balance_check"] = {
                    ...
                    "details": {
                        ...
                        "required": float(self.web3.from_wei(required_balance, "ether")) if value else "N/A"
                    }
                }
            except Exception as e:
    ...
    async def is_healthy(self) -> bool:
        ...
        try:
            ...
            # Check if we can fetch account balance (basic blockchain interaction)
            _ = await self.web3.eth.get_balance(self.account_address)
            
            # Check if circuit breaker is active
            if self.circuit_broken:
                logger.warning(f"Circuit breaker is active: {self.circuit_break_reason}")
                return False
