...
class ExternalAPIManager:
        ...
        try:
            ...
            if symbols_file and symbols_file.exists():
                ...
                with open(symbols_file) as f:
                    ...
                    self.token_symbol_to_address.update(
                        mappings.get("symbol_to_address", {})
                    )
                    self.token_address_to_symbol.update(
                        mappings.get("address_to_symbol", {})
                    )
                    self.symbol_to_api_id.update(mappings.get("symbol_to_api_id", {}))
                    logger.info(
                        f"Loaded {len(self.token_symbol_to_address)} token mappings"
                    )
            else:
    ...
    async def _fetch_price_from_provider(
        self, provider: Provider, symbol: str
    ) -> Optional[float]:
        ...
        try:
            ...
            async with session.get(url, **kwargs) as response:
                if response.status == 200:
                    ...
                    return self._extract_price_from_response(
                        provider.name, data, symbol
                    )
                else:
                    logger.warning(
                        f"{provider.name} API returned status {response.status}"
                    )
                    return None
    ...
    def _extract_price_from_response(
        self, provider_name: str, data: dict, symbol: str
    ) -> Optional[float]:
    ...
    async def _fetch_volume_from_provider(
        self, provider: Provider, symbol: str
    ) -> Optional[float]:
        ...
        try:
            ...
            async with session.get(url, **kwargs) as response:
                if response.status == 200:
                    data = await response.json()
                    return self._extract_volume_from_response(
                        provider.name, data, symbol
                    )
                else:
                    logger.warning(
