...
class MarketMonitor:
    ...
    async def get_token_volume(
        self, token: str, quote_currency: str = "USD", timeframe: str = "24h"
    ) -> Optional[Decimal]:
    ...
    async def predict_price_movement(
        self, token: str, timeframe: str = "1h", use_ml: bool = False
    ) -> Dict[str, Any]:
        ...
        try:
            # Get market features
            await self._get_market_features(token)
            ...
            if use_ml and hasattr(self.api_config, "predict_price"):
                ...
                if prediction:
                    ...
                    if current_price:
                        change = (
                            (float(prediction) - float(current_price))
                            / float(current_price)
                            * 100
                        )
                        direction = (
            ...
            trend_data.get("percent_change", 0)
            ...
            if current_price:
                # Estimate price movement based on signals and current
                # conditions
            ...
            result.update({"prediction": "failed", "error": str(e)})
            return result
    ...
    async def _get_token_price(
        self, token: str, quote_currency: str = "USD"
    ) -> Optional[Decimal]:
        """Internal method to get token price.
        ...
        Returns:
            Token price or None
        """
        return await self.get_token_price(token, quote_currency=quote_currency)

    async def _get_token_volume(
        self, token: str, timeframe: str = "24h"
    ) -> Optional[Decimal]:
        """Internal method to get token volume.
