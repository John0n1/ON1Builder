...
class NotificationManager:
    ...
    def _initialize_channels(self):
        ...
        slack_webhook = os.environ.get("SLACK_WEBHOOK_URL") or (self.config.get("SLACK_WEBHOOK_URL") if self.config else None)
        if slack_webhook:
            self._channels.append(("slack", slack_webhook))
            logger.info("Slack notifications enabled")
            
        # Initialize Email if SMTP settings are available
        smtp_server = os.environ.get("SMTP_SERVER") or (self.config.get("SMTP_SERVER") if self.config else None)
        smtp_port = os.environ.get("SMTP_PORT") or (self.config.get("SMTP_PORT") if self.config else None)
        smtp_username = os.environ.get("SMTP_USERNAME") or (self.config.get("SMTP_USERNAME") if self.config else None)
        smtp_password = os.environ.get("SMTP_PASSWORD") or (self.config.get("SMTP_PASSWORD") if self.config else None)
        alert_email = os.environ.get("ALERT_EMAIL") or (self.config.get("ALERT_EMAIL") if self.config else None)
    ...
    async def send_notification(self, message: str, level: str = "INFO", details: Dict[str, Any] = None) -> bool:
        """
        ...
        environment = os.environ.get("ENVIRONMENT") or (self.config.get("ENVIRONMENT") if self.config else "development")
    ...
    async def _send_email(self, message: str, level: str, config: Dict[str, Any]) -> bool:
        """
...
async def send_alert(message: str, level: str = "INFO", details: Dict[str, Any] = None, config=None):
    """
