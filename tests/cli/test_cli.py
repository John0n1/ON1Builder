...
from on1builder.cli import parse_args, run_command, monitor_command
...
def test_parse_args_with_custom_args():
    ...
    custom_args = ["run", "--config", "custom_config.yaml", "--multi-chain", "--dry-run"]
...
async def test_run_command():
    ...
    with patch('on1builder.config.config.Configuration') as mock_config_class, \
...
async def test_monitor_command():
    ...
    with patch('on1builder.config.config.Configuration') as mock_config_class, \
         patch('on1builder.monitoring.txpool_monitor.TxpoolMonitor') as mock_monitor_class:
