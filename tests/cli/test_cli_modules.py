...
class TestRunCommands:
    ...
    @patch("asyncio.run")
    def test_run_single_chain_command(
        self, mock_asyncio_run, mock_orchestrator_class, mock_load_config
    ):
    ...
    @patch("asyncio.run")
    def test_run_multi_chain_command(
        self, mock_asyncio_run, mock_orchestrator_class, mock_load_config
    ):
...
class TestStatusCommand:
    ...
    @patch("on1builder.cli.status_cmd.load_configuration")
    @patch("on1builder.cli.status_cmd.DatabaseInterface")
    def test_status_command_with_orchestrator(
        self, mock_db_interface, mock_load_config
    ):
        """Test status command when database connection works."""
        mock_load_config.return_value = {"database": {}, "rpc_url": "http://test"}
        mock_db = MagicMock()
...
class TestCLIIntegration:
    ...
    def test_command_error_handling(self):
        ...
        result = self.runner.invoke(
            run_app, ["run", "--config", "/non/existent/file.yaml"]
        )
        assert result.exit_code != 0
