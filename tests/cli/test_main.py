...
async def test_run_single_chain(mock_config):
    ...
    with patch('on1builder.__main__.Configuration', return_value=mock_config), \
...
async def test_run_multi_chain():
    ...
    with patch('on1builder.__main__.MultiChainConfiguration', return_value=mock_multi_config), \
...
def test_main_with_args():
    ...
    try:
        ...
        with patch('on1builder.__main__.run_async') as mock_run_async, \
             patch('on1builder.__main__.run_single_chain') as mock_run_single:
            main()
        ...
        with patch('on1builder.__main__.run_async') as mock_run_async, \
             patch('on1builder.__main__.run_multi_chain') as mock_run_multi:
            main()
            mock_run_async.assert_called_once()
        ...
        with patch('on1builder.__main__.run_async') as mock_run_async, \
             patch('on1builder.__main__.run_monitor') as mock_run_monitor:
            main()
            mock_run_async.assert_called_once()
