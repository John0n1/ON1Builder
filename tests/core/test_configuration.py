...
async def test_load_from_env_method(configuration_instance):
    ...
    with patch("os.getenv") as mock_getenv:
        mock_getenv.side_effect = lambda key: (
            "test_value" if key == "TEST_KEY" else None
        )
...
async def test_save_method(configuration_instance):
    # Test save method
    with patch("builtins.open"), patch("yaml.dump") as mock_yaml_dump:
...
async def test__validate_method(configuration_instance):
    ...
    default_min_profit = (
        0.001  # Match the hardcoded default from Configuration._DEFAULTS
    )
