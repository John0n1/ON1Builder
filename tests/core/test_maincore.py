...
async def test_bootstrap(main_core):
    ...
    with (
        patch(
            "on1builder.config.config.Configuration.load", new_callable=AsyncMock
        ) as mock_load_config,
        patch.object(
            main_core, "_connect_web3", new_callable=AsyncMock
        ) as mock_connect_web3,
        patch("eth_account.Account.from_key", return_value=AsyncMock()),
        patch.object(
            main_core, "_mk_api_config", new_callable=AsyncMock
        ) as mock_mk_api_config,
        patch.object(
            main_core, "_mk_nonce_core", new_callable=AsyncMock
        ) as mock_mk_nonce_core,
        patch.object(
            main_core, "_mk_safety_net", new_callable=AsyncMock
        ) as mock_mk_safety_net,
        patch.object(
            main_core, "_mk_market_monitor", new_callable=AsyncMock
        ) as mock_mk_market_monitor,
        patch.object(main_core, "_mk_txcore", new_callable=AsyncMock) as mock_mk_txcore,
        patch.object(
            main_core, "_mk_txpool_monitor", new_callable=AsyncMock
        ) as mock_mk_txpool_monitor,
        patch.object(
            main_core, "_mk_strategy_net", new_callable=AsyncMock
        ) as mock_mk_strategy_net,
    ):
...
async def test_connect_web3(main_core):
    ...
    with patch.object(
        main_core, "_connect_web3", return_value=mock_web3
    ) as mock_connect:
...
async def test_run(main_core):
    ...
    with (
        patch.object(main_core, "_bootstrap", new_callable=AsyncMock) as mock_bootstrap,
        ...
        )  # Should create at least 3 background tasks
