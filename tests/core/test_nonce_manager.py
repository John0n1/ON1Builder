...
class TestNonceManager:
    ...
    def mock_orchestrator(self):
        ...
        orchestrator.components["db_manager"].load_nonce_state = AsyncMock(
            return_value={}
        )
    ...
    def test_init_with_orchestrator(
        self, nonce_manager, mock_web3, mock_config, mock_orchestrator
    ):
    ...
    def test_init_without_orchestrator(
        self, nonce_manager_no_orchestrator, mock_web3, mock_config
    ):
    ...
    @pytest.mark.asyncio
    async def test_initialize_with_persisted_nonce_state(
        self, nonce_manager, mock_web3
    ):
    ...
    @pytest.mark.asyncio
    async def test_initialize_persisted_nonce_different_chain(
        self, nonce_manager, mock_web3
    ):
    ...
    async def test_get_onchain_nonce_success(self, nonce_manager, mock_web3):
        ...
        nonce = await nonce_manager.get_onchain_nonce(
            "0x742d35Cc6635C0532925a3b8D8b5A8b3F3C4C7c6"
        )
    ...
    async def test_get_onchain_nonce_permanent_failure(self, nonce_manager, mock_web3):
        ...
        with patch("asyncio.sleep", new_callable=AsyncMock):
            with pytest.raises(Exception, match="Permanent error"):
                await nonce_manager.get_onchain_nonce(
                    "0x742d35Cc6635C0532925a3b8D8b5A8b3F3C4C7c6"
                )
    ...
    @pytest.mark.asyncio
    async def test_get_next_nonce_no_address_with_account(
        self, nonce_manager, mock_web3
    ):
    ...
    async def test_get_nonce_alias(self, nonce_manager):
        ...
        with patch.object(
            nonce_manager, "get_next_nonce", new_callable=AsyncMock
        ) as mock_get_next:
    ...
    async def test_track_transaction(self, nonce_manager, mock_web3):
        ...
        with patch.object(
            nonce_manager, "_monitor_transaction", new_callable=AsyncMock
        ):
    ...
    @pytest.mark.asyncio
    async def test_track_transaction_no_address_no_account(
        self, mock_web3, mock_config
    ):
    ...
    async def test_monitor_transaction_failed(self, nonce_manager, mock_web3):
        ...
        with patch.object(
            nonce_manager, "reset_nonce", new_callable=AsyncMock
        ) as mock_reset:
            await nonce_manager._monitor_transaction(tx_hash, address)
    ...
    async def test_monitor_transaction_timeout(self, nonce_manager, mock_web3):
        ...
        with patch.object(
            ...
            with patch(
                "time.time", return_value=time.time() + 1000
            ):  # Simulate timeout
                await nonce_manager._monitor_transaction(tx_hash, address)
    ...
    async def test_close_without_db(self, nonce_manager_no_orchestrator):
        ...
        nonce_manager_no_orchestrator._nonces = {
            "0x742d35Cc6635C0532925a3b8D8b5A8b3F3C4C7c6": 50
        }
