...
class TestTransactionManager:
    ...
    def transaction_manager_minimal(self, mock_web3, mock_account, mock_config):
        ...
        return TransactionManager(
            web3=mock_web3, account=mock_account, configuration=mock_config, chain_id=1
        )
    ...
    def test_init_without_orchestrator(
        self, transaction_manager_minimal, mock_web3, mock_account, mock_config
    ):
    ...
    async def test_initialize_db_error(self, transaction_manager):
        ...
        transaction_manager.db_interface.ensure_tables.side_effect = Exception(
            "DB error"
        )
    ...
    @pytest.mark.asyncio
    async def test_build_transaction_with_nonce_manager(
        self, transaction_manager, mock_nonce_manager
    ):
        """Test building transaction with nonce manager."""
        mock_function = AsyncMock()
        mock_function.build_transaction = AsyncMock(
            return_value={
    ...
    @pytest.mark.asyncio
    async def test_build_transaction_nonce_manager_error(
        self, transaction_manager, mock_nonce_manager, mock_web3
    ):
        """Test building transaction when nonce manager fails."""
        mock_function = AsyncMock()
        mock_function.build_transaction = AsyncMock(
            return_value={"to": "0x742d35Cc6635C0532925a3b8D8b5A8b3F3C4C7c6"}
        )
    ...
    @pytest.mark.asyncio
    async def test_build_transaction_web3_nonce_error(
        self, transaction_manager, mock_nonce_manager, mock_web3
    ):
    ...
    async def test_execute_transaction_success(
        ...
        mock_function.build_transaction = MagicMock(
            return_value={
                "to": "0x742d35Cc6635C0532925a3b8D8b5A8b3F3C4C7c6",
                "gas": 21000,
            }
    ...
    @pytest.mark.asyncio
    async def test_execute_transaction_safety_check_failure(
        self, transaction_manager, mock_safety_guard
    ):
        ...
        mock_safety_guard.check_transaction_safety.return_value = (
            False,
            "Safety check failed",
        )
    ...
    @pytest.mark.asyncio
    async def test_execute_transaction_without_safety_guard(
        self, transaction_manager_minimal
    ):
    ...
    async def test_wait_for_transaction_receipt_success(
        self, transaction_manager, mock_web3
    ):
        ...
        transaction_manager.web3.eth.get_transaction_receipt = AsyncMock(
            return_value=receipt
        )

        result = await transaction_manager.wait_for_transaction_receipt(
    ...
    @pytest.mark.asyncio
    async def test_wait_for_transaction_receipt_timeout(
        self, transaction_manager, mock_web3
    ):
        ...
        with pytest.raises(asyncio.TimeoutError):
            await transaction_manager.wait_for_transaction_receipt(
                "0x1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef",
                timeout=0.1,
            )
    ...
    @pytest.mark.asyncio
    async def test_get_eth_balance_default_address(
        self, transaction_manager, mock_web3
    ):
    ...
    async def test_simulate_transaction_success(self, transaction_manager, mock_web3):
        ...
        success, error, simulation_data = (
            await transaction_manager.simulate_transaction(tx)
        )
    ...
    @pytest.mark.asyncio
    async def test_optimize_gas_price_no_external_api(
        self, transaction_manager, mock_web3
    ):
    ...
    @pytest.mark.asyncio
    async def test_estimate_transaction_cost_gas_error(
        self, transaction_manager, mock_web3
    ):
    ...
    @pytest.mark.asyncio
    async def test_withdraw_eth_insufficient_balance(
        self, transaction_manager, mock_web3
    ):
        ...
        transaction_manager.web3.eth.get_balance.return_value = 1000  # Very small amount
        
        # Create a proper async function for gas_price
        async def mock_gas_price():
            return 20000000000
            
        transaction_manager.web3.eth.gas_price = mock_gas_price()
    ...
    def test_shared_components_access(self, transaction_manager, mock_orchestrator):
        ...
        assert (
            transaction_manager.abi_registry
            == mock_orchestrator.components["abi_registry"]
        )
        assert (
            transaction_manager.db_interface
            == mock_orchestrator.components["db_interface"]
        )
        assert (
    ...
    async def test_prepare_flashloan_transaction_basic(self, transaction_manager):
        ...
        with patch.object(transaction_manager, '_validate_flashloan_safety', return_value=None), \
             patch.object(transaction_manager,
             '_ensure_flashloan_contract_deployed', return_value="0x742d35Cc6635C0532925a3b8D8b5A8b3F3C4C7c6"):
            
            result = await transaction_manager.prepare_flashloan_transaction(
                assets=asset,
                amounts=amount
            )
            
            assert "flashloan_contract" in result
            assert "assets" in result
            assert result["assets"][0] == asset

    @pytest.mark.asyncio
    async def test_cancel_transaction_with_custom_gas(self, transaction_manager, mock_web3):
        """Test transaction cancellation with custom gas price."""
        nonce = 5
    ...
    async def test_get_flashloan_contract_info(self, transaction_manager):
        ...
        # Check for keys that are actually returned by the method
    ...
    async def test_validate_contract_integration(self, transaction_manager):
        ...
        assert isinstance(result, dict)
        # Check for keys that are actually returned
        assert "abi_registry" in result
        assert "web3_connection" in result
        assert "account_ready" in result
    ...
    async def test_emergency_stop(self, transaction_manager):
        ...
        assert "pending_transactions_cancelled" in result
        assert "contracts_cleaned" in result
    ...
    async def test_send_bundle(self, transaction_manager, mock_web3):
        ...
        assert result == "1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef,1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef"
    ...
    async def test_execute_sandwich_attack(self, transaction_manager, mock_web3):
        ...
        result = await transaction_manager.execute_sandwich_attack(
            target_tx, strategy="custom"
        )
        
        # execute_sandwich_attack returns a tuple (front_run_hash, back_run_hash)
    ...
    async def test_get_integration_status(self, transaction_manager):
        ...
        with patch.object(transaction_manager, 'validate_contract_integration', return_value={"aave_integration": True}):
            ...
            assert isinstance(result, dict)
            assert "transaction_core" in result
