...
def transaction_core(configuration):
    ...
    return TransactionCore(web3, account, configuration)


@pytest.mark.asyncio
async def test_initialize(transaction_core):
    with patch.object(
        transaction_core, "initialize", new_callable=AsyncMock
    ) as mock_initialize:
...
async def test_build_transaction(transaction_core):
    ...
    with patch.object(
        transaction_core, "build_transaction", new_callable=AsyncMock
...
async def test_execute_transaction(transaction_core):
    ...
    with patch.object(
        transaction_core, "execute_transaction", new_callable=AsyncMock
...
async def test_handle_eth_transaction(transaction_core):
    ...
    with patch.object(
        transaction_core, "handle_eth_transaction", new_callable=AsyncMock
    ) as mock_handle_eth_transaction:
...
async def test_simulate_transaction(transaction_core):
    ...
    with patch.object(
        transaction_core, "simulate_transaction", new_callable=AsyncMock
    ) as mock_simulate_transaction:
...
async def test_prepare_flashloan_transaction(transaction_core):
    ...
    with patch.object(
        transaction_core, "prepare_flashloan_transaction", new_callable=AsyncMock
    ) as mock_prepare_flashloan_transaction:
        await transaction_core.prepare_flashloan_transaction(
            flashloan_asset, flashloan_amount
        )
        mock_prepare_flashloan_transaction.assert_called_once_with(
            flashloan_asset, flashloan_amount
        )
...
async def test_send_bundle(transaction_core):
    ...
    with patch.object(
        transaction_core, "send_bundle", new_callable=AsyncMock
    ) as mock_send_bundle:
...
async def test_front_run(transaction_core):
    target_tx = {"tx_hash": "0xHash", "value": 1000}
    with patch.object(
        transaction_core, "front_run", new_callable=AsyncMock
    ) as mock_front_run:
...
async def test_back_run(transaction_core):
    target_tx = {"tx_hash": "0xHash", "value": 1000}
    with patch.object(
        transaction_core, "back_run", new_callable=AsyncMock
    ) as mock_back_run:
...
async def test_execute_sandwich_attack(transaction_core):
    ...
    strategy = "default"
    with patch.object(
        transaction_core, "execute_sandwich_attack", new_callable=AsyncMock
    ) as mock_execute_sandwich_attack:
...
async def test_cancel_transaction(transaction_core):
    nonce = 1
    with patch.object(
        transaction_core, "cancel_transaction", new_callable=AsyncMock
    ) as mock_cancel_transaction:
...
async def test_withdraw_eth(transaction_core):
    with patch.object(
        transaction_core, "withdraw_eth", new_callable=AsyncMock
    ) as mock_withdraw_eth:
...
async def test_transfer_profit_to_account(transaction_core):
    ...
    account = "0xAccount"
    with patch.object(
        transaction_core, "transfer_profit_to_account", new_callable=AsyncMock
    ) as mock_transfer_profit_to_account:
