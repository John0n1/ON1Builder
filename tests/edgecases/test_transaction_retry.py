...
def transaction_core(config):
    with patch("on1builder.core.transaction_core.AsyncWeb3") as mock_web3_class:
        ...
        mock_web3.to_wei = lambda amount, unit: (
            amount * 10**9 if unit == "gwei" else amount
        )  # Mock to_wei function
        ...
        mock_nonce_core.get_nonce = AsyncMock(
            return_value=123
        )  # Mock async nonce getter
...
async def test_transaction_retry_on_timeout(transaction_core):
    ...
    mock_send = AsyncMock(side_effect=[TimeExhausted("Transaction timed out"), tx_hash])
    ...
    transaction_core.send_signed = AsyncMock(
        side_effect=[TimeExhausted("Transaction timed out"), tx_hash]
    )
...
async def test_transaction_retry_max_exceeded(transaction_core):
    ...
    assert (
        transaction_core.send_signed.call_count
        == transaction_core.config.MEMPOOL_MAX_RETRIES
    )
