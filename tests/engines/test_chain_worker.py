...
async def test_initialize(chain_worker):
    ...
    mocks['init_web3'] = patch.object(chain_worker, "_initialize_web3", new_callable=AsyncMock)
    ...
    # Mock DB manager
    mocks['db_manager'] = patch("on1builder.engines.chain_worker.get_db_manager", new_callable=AsyncMock)
    ...
    mocks['test_initialize'] = patch.object(chain_worker, "_test_initialize", new_callable=AsyncMock)
    mock_test_initialize = mocks['test_initialize'].start()
    mock_test_initialize.return_value = True
    
    # Mock wallet balance and gas price
    mocks['get_wallet_balance'] = patch.object(chain_worker, "get_wallet_balance", new_callable=AsyncMock)
    mock_get_wallet_balance = mocks['get_wallet_balance'].start()
    mock_get_wallet_balance.return_value = 1.5
    
    mocks['get_gas_price'] = patch.object(chain_worker, "get_gas_price", new_callable=AsyncMock)
    mock_get_gas_price = mocks['get_gas_price'].start()
    mock_get_gas_price.return_value = 25.0
    
    # Add a mock for _get_monitored_tokens
    mocks['get_monitored_tokens'] = patch.object(chain_worker, "_get_monitored_tokens", new_callable=AsyncMock)
...
async def test_initialize_failure(chain_worker):
    ...
    with patch.object(chain_worker, "_initialize_web3", new_callable=AsyncMock) as mock_init_web3:
...
async def test_get_wallet_balance(chain_worker):
    ...
    chain_worker.web3.eth.get_balance = AsyncMock(return_value=1500000000000000000)  # 1.5 ETH in wei
