...
async def test_ensure_profit(safety_net):
    ...
    with patch.object(safety_net.api_config, 'get_real_time_price', new_callable=AsyncMock) as mock_get_real_time_price, \
         patch.object(safety_net, '_calculate_gas_cost', new_callable=AsyncMock) as mock_calculate_gas_cost, \
         patch.object(safety_net, 'adjust_slippage_tolerance', new_callable=AsyncMock) as mock_adjust_slippage_tolerance, \
         patch.object(safety_net, 'get_dynamic_gas_price', new_callable=AsyncMock) as mock_dynamic_gas_price, \
         patch.object(safety_net, 'get_network_congestion', new_callable=AsyncMock) as mock_network_congestion:
...
async def test_check_transaction_safety(safety_net):
    ...
    with patch.object(safety_net, 'get_dynamic_gas_price', new_callable=AsyncMock) as mock_get_dynamic_gas_price, \
         patch.object(safety_net.api_config, 'get_real_time_price', new_callable=AsyncMock) as mock_get_real_time_price, \
         patch.object(safety_net, 'adjust_slippage_tolerance', new_callable=AsyncMock) as mock_adjust_slippage_tolerance, \
         patch.object(safety_net, '_calculate_gas_cost', new_callable=AsyncMock) as mock_calculate_gas_cost, \
         patch.object(safety_net, 'get_network_congestion', new_callable=AsyncMock) as mock_get_network_congestion, \
         patch.object(safety_net, 'ensure_profit', new_callable=AsyncMock) as mock_ensure_profit, \
         patch.object(safety_net.web3.eth, 'get_balance', new_callable=AsyncMock) as mock_get_balance:
