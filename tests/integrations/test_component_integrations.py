...
async def config():
    ...
    config = Configuration(skip_env=True)  # Skip environment variables to avoid override
...
async def db_manager(config, temp_db_file):
    """Create a test database manager with a temporary file-based SQLite database."""
    ...
    class TestDBManager:
        ...
        async def save_transaction(self, tx_hash, chain_id, from_address, to_address, value, gas_price, gas_used=None, 
                                block_number=None, status=None, data=None):
        ...
        async def save_profit_record(self, tx_hash, chain_id, profit_amount, token_address, strategy):
...
async def test_abi_registry_initialization(abi_registry):
    ...
    # Mock is_healthy
    with patch.object(registry, 'is_healthy', new_callable=AsyncMock) as mock_is_healthy:
...
async def test_logger_and_db_integration():
    ...
    with patch('on1builder.persistence.db_manager.HAS_SQLALCHEMY', True), \
         patch('on1builder.persistence.db_manager.create_async_engine') as mock_engine, \
         patch('on1builder.persistence.db_manager.setup_logging', return_value=test_logger), \
