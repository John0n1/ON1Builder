...
@pytest.fixture
def txpool_monitor(
    web3, safety_net, nonce_core, api_config, configuration, market_monitor
):
...
async def test_handle_new_transactions(txpool_monitor):
    ...
    with patch.object(
        txpool_monitor, "_process_transaction", new_callable=AsyncMock
...
async def test_monitor_memory(txpool_monitor):
    with patch(
        "on1builder.monitoring.txpool_monitor.psutil.virtual_memory"
...
async def test_get_dynamic_gas_price(txpool_monitor):
    with patch.object(
        txpool_monitor.web3.eth, "get_block", new_callable=AsyncMock
    ) as mock_get_block:
        mock_get_block.return_value = {"baseFeePerGas": 1000000000}
        await txpool_monitor.get_dynamic_gas_price()
