...
async def test_save_transaction(db_manager):
    ...
    result = await db_manager.save_transaction(
        ...
        gas_used, block_number, status, data
    )
...
async def test_save_profit_record(db_manager):
    ...
    result = await db_manager.save_profit_record(
...
async def test_get_transaction(db_manager):
    ...
    mock_tx.to_dict = MagicMock(return_value={
...
async def test_get_profit_summary(db_manager):
    ...
    mock_session.execute.side_effect = [
        mock_profit_result, mock_gas_result, mock_success_result, mock_total_result
    ]
    ...
    result = await db_manager.get_profit_summary(chain_id, address, start_time, end_time)
