...
class TestMarketDataFeed:
    ...
    def test_init(self, market_data_feed, mock_web3, global_settings, api_settings):
        ...
        assert (
            market_data_feed._cleanup_interval == global_settings.cache_cleanup_interval
        )
    ...
    async def test_initialize(self, market_data_feed):
        """Test market data feed initialization."""
        with patch.object(
            market_data_feed, "api_config_client_session"
        ) as mock_session:
    ...
    async def test_get_token_price_cache_expired(self, market_data_feed):
        ...
        # Mock API call
        with patch.object(
            market_data_feed, "api_config_real_time_price", new_callable=AsyncMock
        ) as mock_api:
            ...
            assert market_data_feed._price_cache[cache_key]["price"] == Decimal(
                "3500.00"
            )
    ...
    async def test_get_market_trend_with_history(self, market_data_feed):
        ...
        with patch.object(
            market_data_feed, "api_config_price_history", new_callable=AsyncMock
        ) as mock_history_api:
    ...
    async def test_get_market_trend_fallback(self, market_data_feed):
        ...
        with patch.object(
            ...
            with patch.object(
                market_data_feed, "get_token_price", new_callable=AsyncMock
            ) as mock_price:
    ...
    async def test_cache_cleanup_triggers(self, market_data_feed):
        ...
        market_data_feed._last_cache_cleanup = (
            time.time() - market_data_feed._cleanup_interval - 1
        )

        with patch.object(
            market_data_feed, "_cleanup_cache", new_callable=AsyncMock
        ) as mock_cleanup:
            with patch.object(
                market_data_feed, "api_config_real_time_price", new_callable=AsyncMock
            ) as mock_api:
    ...
    def test_metrics_tracking(self, market_data_feed):
        ...
        market_data_feed.metrics["test_metric"] = (
            market_data_feed.metrics.get("test_metric", 0) + 1
        )
