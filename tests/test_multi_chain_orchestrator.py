...
class TestMultiChainOrchestrator:
    ...
    async def test_initialize_success(self, orchestrator, mock_config):
        ...
        with patch.object(
            orchestrator, "_init_chain_worker", new_callable=AsyncMock
        ) as mock_init:
    ...
    @pytest.mark.asyncio
    async def test_init_chain_worker_success(
        self, orchestrator, mock_config, mock_chain_config
    ):
    ...
    @pytest.mark.asyncio
    async def test_init_chain_worker_failure(
        self, orchestrator, mock_config, mock_chain_config
    ):
    ...
    @pytest.mark.asyncio
    async def test_init_chain_worker_exception(
        self, orchestrator, mock_config, mock_chain_config
    ):
