"""Implementation of Telegram notification functionality and tests for the
notification system."""
...
async def test_send_notification_all_channels(notification_manager):
    ...
    with (
        patch.object(
            notification_manager, "_send_discord", return_value=True
        ) as mock_discord,
        patch.object(
            notification_manager, "_send_slack", return_value=True
        ) as mock_slack,
        patch.object(
            notification_manager, "_send_telegram", return_value=True
        ) as mock_telegram,
        patch.object(
            notification_manager, "_send_email", return_value=True
        ) as mock_email,
    ):
        ...
        mock_email.assert_called_once_with(
            message, f"[{level}] ON1Builder Notification"
        )
...
async def test_send_notification_level_filtering(notification_manager):
    ...
    with (
        patch.object(notification_manager, "_send_discord") as mock_discord,
...
async def test_get_notification_manager_with_custom_config():
    ...
    with patch(
        "on1builder.utils.notifications.NotificationManager"
